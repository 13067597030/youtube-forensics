"""Probe list_creator_videos filters."""
import re
...
filters = {
    "and_filter": {"and": {"operands": [{"channelIdIs": {"value": CHANNEL}}, {"videoOriginIs": {"value": "VIDEO_ORIGIN_UPLOAD"}}]}},
    "channel_only": {"channelIdIs": {"value": CHANNEL}},
    "none": {},
}
for name, filt in filters.items():
    body = {"context": ctx, "order": "VIDEO_ORDER_DISPLAY_TIME_DESC", "pageSize": 5, "mask": mask}
    if filt:
        body["filter"] = filt
    r = s.client.post(f"{O}/youtubei/v1/creator/list_creator_videos", params={"key": key}, json=body, headers=h)
    n = 0
    try:
        n = len(r.json().get("videos") or [])
    except Exception:
        pass
    print(name, r.status_code, "videos", n)
s.close()
